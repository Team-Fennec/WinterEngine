...
namespace Veldrid.Sdl2
{
    public static unsafe partial class Sdl2Native
    {
        ...
        [UnmanagedFunctionPointer(CallingConvention.Cdecl)]
        private delegate void SDL_ShowSimpleMessageBox_t(SDL_MessageBoxFlags flags, byte* title, byte* message, SDL_Window* Sdl2Window);
        private static SDL_ShowSimpleMessageBox_t s_showSimpleMessageBox = LoadFunction<SDL_ShowSimpleMessageBox_t>("SDL_ShowSimpleMessageBox");
        public static void SDL_ShowSimpleMessageBox(SDL_MessageBoxFlags flags, string title, string message, SDL_Window* Sdl2Window)
        {
            byte* utf8BytesTitle;
            byte* utf8BytesMessage;
            if (title != null)
            {
                int byteCount = Encoding.UTF8.GetByteCount(title);
                if (byteCount == 0)
                {
                    byte zeroByte = 0;
                    utf8BytesTitle = &zeroByte;
                }
                else
                {
                    byte* utf8BytesAlloc = stackalloc byte[byteCount + 1];
                    utf8BytesTitle = utf8BytesAlloc;
                    fixed (char* titlePtr = title)
                    {
                        int actualBytes = Encoding.UTF8.GetBytes(titlePtr, title.Length, utf8BytesTitle, byteCount);
                        utf8BytesTitle[actualBytes] = 0;
                    }
                }
            }
            else
            ...
            if (message != null)
            {
                ...
                {
                    byte zeroByte = 0;
                    utf8BytesMessage = &zeroByte;
                }
                else
                {
                    byte* utf8BytesAlloc = stackalloc byte[byteCount + 1];
                    utf8BytesMessage = utf8BytesAlloc;
                    fixed (char* titlePtr = message)
                    {
                        int actualBytes = Encoding.UTF8.GetBytes(titlePtr, message.Length, utf8BytesMessage, byteCount);
                        utf8BytesMessage[actualBytes] = 0;
                    }
                }
            }
            else
            {
                utf8BytesMessage = null;
            }

            s_showSimpleMessageBox(flags, utf8BytesTitle, utf8BytesMessage, Sdl2Window);
        }
